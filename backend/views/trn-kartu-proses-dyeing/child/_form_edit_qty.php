...
<div class="edit-qty-form">
    <?php $form = ActiveForm::begin([
        'id' => 'edit-qty-form',
        'action' => ['trn-kartu-proses-dyeing-item/edit-qty', 'id' => $model->id],
        'method' => 'post',
    ]); ?>

    <!-- hidden input untuk relasi induk -->
    <?= Html::activeHiddenInput($model, 'kartu_process_id', ['id' => 'kartu-process-id']) ?>
...
$(document).on('click', '.pilih-stock', function(e) {
    ...
    // isi hidden input stock_id
    $('#stock-id').val(stockId);

    $('#lot_lusi').val(lot_lusi);
    $('#lot_pakan').val(lot_pakan);
    $('#panjang-m').val(panjang);
    $('#no_document').val(no_document);
    
    alert('Stock dipilih: ' + stockId + '\\nNo Document: ' + no_document + '\\nLot Lusi: ' + lot_lusi + '\\nLot Pakan: ' + lot_pakan + '\\nPanjang: ' + panjang);
});

// cegah Enter hanya di input modal #panjang-m
$('#panjang-m').on('keypress', function(e) {
    if (e.which === 13) {
        e.preventDefault(); // jangan submit form modal saat tekan Enter
        return false;
    }
});
JS;
