    ...
    <table>
        ...
        <tbody>
            <?php if (empty($records)): ?>
                <tr>
                    <td colspan="10" class="text-center" style="font-style: italic; color: #7f8c8d; padding: 20px;">
            ...
            <?php else: ?>
                <?php $no = 1; foreach ($records as $record): ?>
                    <tr>
                        ...
                        <td class="text-center"><?= Html::encode(date('d-M-Y', strtotime($record['tanggal_wo']))) ?></td>
                        <td><?= Html::encode($record['buyer']) ?></td>
                        <td><?= Html::encode($record['wo_no']) ?></td>
                        <td><?= Html::encode($record['motif']) ?></td>
                        <td><?= Html::encode($record['warna']) ?></td>
                        <td class="text-center" style="font-weight: bold;"><?= Html::encode($record['nk']) ?></td>
                        <td class="text-center" style="color: #27ae60; font-weight: bold;"><?= Html::encode(date('d-M-Y', strtotime($record['buka_greige']))) ?></td>
                        <td class="text-center" style="font-weight: bold; color: <?= $record['tgl_packing'] === 'Belum Packing' ? '#e74c3c' : '#7f8c8d' ?>;">
