...
class TrnInspectingController extends Controller
{
                ...
                try {
                    ...
                    foreach (Json::decode(Yii::$app->request->post('items')) as $item) {
                        ...
                        $existingDefects = DefectInspectingItem::find()
                            ->where(['inspecting_item_id' => $query_one->id])
    ...
    public function actionUpdate($id)
    {
        ...
        if ($model->status != $model::STATUS_DRAFT && $model->status != $model::STATUS_APPROVED_PARTIAL && $model->status != $model::STATUS_APPROVED && $model->status != $model::STATUS_DELIVERED && !$hasPostedItemsNotReceived) {
        ...
        foreach ($inspectItems as $item) {
            ...
            $items[] = [
                ...
                'grade'      => $item->grade,
                'gradeLabel' => InspectingItem::gradeOptions()[$item->grade] ?? '-',
                'defect'     => !empty($item->defect) ? $item->defect : implode(',', array_map(function($d){
                    return $d->mstKodeDefect ? $d->mstKodeDefect->no_urut : '';
                }, $item->defectInspectingItems)),
                'lot_no'     => $item->lot_no,
                'ukuran'     => $item->qty,
                'join_piece' => $item->join_piece,
                'keterangan' => $item->note,
            ...
            if ($modelHeader->load(Yii::$app->request->post()) && $modelHeader->validate()) {
                ...
                try {
                    ...
                    foreach ($itemsData as $item) {
                        ...
                        // === PERSIST DEFECTS ===
                        $existingDefects = DefectInspectingItem::find()
                            ->where(['inspecting_item_id' => $itemModel->id])
                            ->indexBy('mst_kode_defect_id')
                            ->all();
    ...
    public function actionGantiWarna($id)
    {
        if(Yii::$app->request->isAjax){
            ...
            if(in_array($model->status, [$model::STATUS_DELIVERED, $model::STATUS_APPROVED_PARTIAL])){
                throw new ForbiddenHttpException('Status tidak valid, Diterima Gudang Jadi atau Diterima Partial tidak bisa diganti warnanya.');
            }
