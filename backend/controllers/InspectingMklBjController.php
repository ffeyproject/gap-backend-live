...
class InspectingMklBjController extends Controller
{
    ...
    public function actionPosting($id)
    {
        ...
        try {
            ...
            $selectedJoinPieces = InspectingMklBjItems::find()
                ->select('join_piece')
            ...
            if (!empty($selectedJoinPieces)) {
                $siblingItemIds = InspectingMklBjItems::find()
                    ->select('id')
            ...
            InspectingMklBjItems::updateAll(['is_posted' => true, 'posted_at' => $postingDate], ['id' => $allItemIdsToPost, 'inspecting_id' => $model->id]);
    ...
    public function actionUnpostItem($id)
    {
        ...
        if (!empty($item->join_piece)) {
            $siblingItems = InspectingMklBjItems::find()
            ...
            foreach ($siblingItems as $sibling) {
                ...
                $isReceived = \common\models\ar\TrnGudangJadi::find()->where(['id_from' => $sibling->id, 'trans_from' => 'MKL'])->exists();
                if ($isReceived) {
        ...
        InspectingMklBjItems::updateAll(['is_posted' => false, 'posted_at' => null], ['id' => $allItemIds, 'inspecting_id' => $item->inspecting_id]);
    ...
    public function actionHapusSemuaDefect($id)
    {
        ...
        $items = $model->getItems()->select('id')->asArray()->all();
        $itemIds = array_column($items, 'id');

        if (empty($itemIds)) {
            Yii::$app->session->setFlash('info', 'Tidak ada item untuk dihapus defect-nya.');
            return $this->redirect(['view', 'id' => $model->id]);
        }

        $transaction = Yii::$app->db->beginTransaction();
        try {
            // Hapus semua defect berdasarkan semua item Inspecting terkait
            $deleted = DefectInspectingItem::deleteAll([
                'inspecting_mklbj_item_id' => $itemIds
            ]);

            // Update kolom updated_at & updated_by (integer timestamp)
            ...
            if ($deleted) {
                Yii::$app->session->setFlash('success', "Semua defect ($deleted baris) berhasil dihapus.");
            } else {
