...
class TrnGudangJadiSearch extends TrnGudangJadi
{
    ...
    public function search($params)
    {
        ...
        // $query->select([
        //     'trn_gudang_jadi.*',
            // 'id_asal' => new Expression("(SELECT COALESCE(
            //     (SELECT x.inspecting_id FROM inspecting_item x WHERE x.id = trn_gudang_jadi.id_from AND trn_gudang_jadi.trans_from = 'INS'),
            //     (SELECT y.inspecting_id FROM inspecting_mkl_bj_items y WHERE y.id = trn_gudang_jadi.id_from AND trn_gudang_jadi.trans_from = 'MKL'),
            //     NULL
            // ))")
        // ]);
