 ...
 */
class TrnMoSearchArray extends TrnMo
{
    public $from_date;
    public $to_date;
    public $dateRange;
    ...
    {
        $query = TrnMo::find()
        ->joinWith(['sc.cust', 'sc.marketing as mkt', 'scGreige.greigeGroup', 'createdBy as crt'])
        
        // Subquery untuk `trn_mo_color`
        ...
        $query->select([
            'trn_mo.*',           // Data dari tabel utama `trn_mo`
            'MAX(tmc.color_qty) as color_qty',  // Menggunakan MAX untuk menghindari error group by
            'MAX(twc.wo_color_qty) as wo_color_qty',
            'MAX(tmc.color_qty - COALESCE(twc.wo_color_qty, 0)) as wo_sisa_batch',
        ]);
        $query->where('color_qty > wo_color_qty');
        $query->orWhere('wo_color_qty IS NULL');

        // Melakukan group hanya berdasarkan `trn_mo.id`
        $query->groupBy('trn_mo.id');
    ...
    //         'asc' => [new \yii\db\Expression('trn_mo.approved_at IS NULL, trn_mo.approved_at DESC')],
    //         'desc' => [new \yii\db\Expression('trn_mo.approved_at DESC')],
    //     ];
    ...
    //     }
        ...
        $dataProvider = new ArrayDataProvider([
            ...
            'sort' => [
                'attributes' => ['date','id','no_urut','dateRange','nomorSc','creatorName','customerName','marketingName','scGreigeNamaKain','no'],
